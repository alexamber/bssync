...
- **Env vars now actually override yaml values.** `load_config` previously documented "env overrides yaml" but implemented "yaml wins; env is fallback" — so a user with `token_id` in both places would silently use the yaml value. CI and Claude Desktop `env:` block usage now work as documented; yaml is still the source when env vars are unset.
